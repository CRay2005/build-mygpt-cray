...
# -----------------------------------------------------------------------------
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        #为两组参数设置不同的权重衰减策略
        #维度≥2的参数（如权重矩阵）应用权重衰减
        #维度<2的参数（如偏置项和层归一化参数）不应用权重衰减
        optim_groups = [
...
class DataLoaderLite:
    ...
    def next_batch(self):
        ...
        self.current_position += B * T * self.num_processe
...
#beta2=0.9，一阶矩（均值）的衰减率（默认值）
#beta2=0.95，二阶矩（未中心化方差）的衰减率。比默认的 0.999 衰减更慢，保留更多近期梯度信息，适合需要快速适应的场景（如大模型训练）
...
for step in range(max_steps):
    ...
    #梯度裁剪（Gradient Clipping）​​ 函数，用于防止梯度爆炸（Exploding Gradients）问题
